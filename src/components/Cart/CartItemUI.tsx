...
import { useAddProductSaveForLaterMutation, useViewSaveLaterProductsQuery } from "../../../gql/graphql";
  ...
  useEffect(() => {
    if (data?.sflView) {
      const formattedData = data.sflView.map((sfl) => ({
        ...
        price: sfl?.product?.pricing?.priceRangeUndiscounted?.start?.net?.amount, 
        currency: sfl?.product?.pricing?.priceRangeUndiscounted?.start?.net?.currency,
      }));
